...
class PolicyEnforcementFunctionMiddleware(FunctionMiddleware, _SecurityScopeBinding):
        ...
        *,
        ...
        Args:
            ...
            max_pending_approvals: Maximum number of pending approvals to retain. When exceeded,
                ...
                Defaults to 1000.
            security_scope: Optional security scope for state management.
            session_state_key: Key for session state storage.
        """
    ...
    def _store_pending_approval(self, approval_id: str, record: _PendingPolicyApproval) -> None:
        """Store one pending approval in detached JSON-compatible form with FIFO eviction."""
        ...
        if (
            ...
            and approval_id not in self._scope.pending_approvals
            and len(self._scope.pending_approvals) >= self._max_pending_approvals
        ):
            # Evict oldest (first) entry
            oldest_approval_id = next(iter(self._scope.pending_approvals))
            del self._scope.pending_approvals[oldest_approval_id]
            logger.debug(
